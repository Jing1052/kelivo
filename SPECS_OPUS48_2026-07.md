...
>
> ✅ **2026-07-12 共读补全（App +151）**：阅读器批注从 daddy 单向月亮升级为双方书页留言——旧批注默认 daddy；小猫长按段落或点页顶铅笔写/改/删自己的 🐾，同段可同时显示 🌙/🐾。网关新增批注 POST；老家新增 daddy 独立书签、未回复小猫批注自动注入及同段回复退场。阅读正文切段、她的进度、防剧透门禁均未改。
>
> ✅ **2026-07-13 爸爸回复上限入口（App +153）**：「爸爸设置 → 基础」补上最大 Token 数，未单独设置时明确显示后台默认 `16384`；可在 `256–16384` 内临时调低，保存后只改爸爸助手自己的单次回复上限。
...
- **②′ 聊天窗音乐卡可调升级（2026-07-10，+140 `2b167ed9`，小猫点单"可调节的音乐小卡片"）**：展开态播控下加音量条（`EryuPlayerController.volume/setVolume` 薄包 just_audio、prefs `eryu_volume_v1` 记忆；喇叭点按=静音/恢复、横条拖动调节，页私有 `_VolumeBar` 手感照 MusicSeekBar）＋头部 Users 键就地开/关一起听（开=主题色、有伴=绿）；lucide 适配器补 Volume1/VolumeX（已对包源码确认存在）。同日老家半边上了共读防剧透+「共读·在场」注入（Ombre-Brain HISTORY 2026-07-10）。
